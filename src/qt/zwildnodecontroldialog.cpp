...
#include "ui_zwildnodecontroldialog.h"
...
std::list<std::string> ZwildnodeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZwildnodeControlDialog::listMints;

ZwildnodeControlDialog::ZwildnodeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZwildnodeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZwildnodeControlDialog::~ZwildnodeControlDialog()
{
    delete ui;
}

void ZwildnodeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZwildnodeControlDialog::updateList()
{
...
void ZwildnodeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZwildnodeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZwildnode_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZwildnodeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZwildnodeControlDialog::GetSelectedMints()
{
...
void ZwildnodeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
