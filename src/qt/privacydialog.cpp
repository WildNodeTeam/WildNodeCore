...
#include "zwildnodecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zwld ought to be enough for anybody." - Bill Gates, 2017
    ui->zwldpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzwldSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzwld->setEnabled(false);
        ui->pushButtonMintzwld->setToolTip(tr("zwld is currently disabled due to maintenance."));

        ui->pushButtonSpendzwld->setEnabled(false);
        ui->pushButtonSpendzwld->setToolTip(tr("zwld is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zwldpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzwld_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zwld is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zwld...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zwld in ") +
...
void PrivacyDialog::on_pushButtonSpendzwld_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zwld is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zwld
        sendzwld();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zwld
    sendzwld();
}

void PrivacyDialog::on_pushButtonZwildnodeControl_clicked()
{
    ZwildnodeControlDialog* zwldControl = new ZwildnodeControlDialog(this);
    zwldControl->setModel(walletModel);
    zwldControl->exec();
}

void PrivacyDialog::setZwildnodeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzwldSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzwld()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid wildnode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zwldpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zwld is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " wildnode </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zwld</b>";
    ...
    // use mints from zwld selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZwildnodeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZwildnodeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zwildnode selector in case it was used
    ZwildnodeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zwld Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " wildnode, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zwld Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zwld </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zwld "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zwld "));
    ui->labelzwldAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzwldSyncStatus->setVisible(fShow);
}
